...
export class GameModeSelector extends LitElement {
   ...
   * until the first snapshot, but from then on the heading and its link stay
   * even with nothing queued — that link is the only way to the browser.
   */
      ...
      <section class="flex flex-col gap-3 min-w-0 min-h-0">
        <div class="flex items-center justify-between gap-2">
          <div class="flex items-center gap-1.5 min-w-0">
            <h2
              class="text-sm font-bold uppercase tracking-widest text-white/70 truncate"
            >
              ${translateText("public_lobby.upcoming")}
            </h2>
            ${this.renderQueueHelp()}
          </div>
          <button
            @click=${this.openDetailed}
            class="shrink-0 rounded-lg bg-white/[0.06] px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-white/85 hover:bg-malibu-blue hover:text-white transition-colors"
          >
    ...
    return html`
      <div class="group/queue-help relative shrink-0">
        ...
        <div
          role="tooltip"
          id="upcoming-help"
          class="pointer-events-none absolute left-0 lg:left-auto lg:right-0 top-6 z-20 ${this
            .helpOpen
            ? "flex"
